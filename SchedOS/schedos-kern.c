...
start(void)
{
	...
	proc_array[1].p_count = 1;
	proc_array[2].p_count = 0;
	proc_array[3].p_count = 0;
	proc_array[4].p_count = 0;
	
	proc_array[1].p_share = 1;
	proc_array[2].p_share = 1;
	proc_array[3].p_share = 2;
	proc_array[4].p_share = 3;

	proc_array[1].p_tickets = 10;
	proc_array[2].p_tickets = 20;
	proc_array[3].p_tickets = 30;
	proc_array[4].p_tickets = 40;
...
schedule(void)
{
	...
	if (scheduling_algorithm == 0)
	  ...
	  unsigned int minPriority = 0xFFFFFFFF;
	  int i;
	  for(i = 0; i < NPROCS; i++) {
	    if(proc_array[i].p_state == P_RUNNABLE 
	       && proc_array[i].p_priority < minPriority) {
	      minPriority = proc_array[i].p_priority;
	    }
	  }
	  while(1) {
	    pid = (pid + 1) % NPROCS;
	    if(proc_array[pid].p_state == P_RUNNABLE 
	       && proc_array[pid].p_priority == minPriority) {
	      run(&proc_array[pid]);
	    }
	  }
	  ...
	  for(i = 0; i < NPROCS; i++) {
	    if(proc_array[i].p_state == P_RUNNABLE) {
	      numTickets += proc_array[i].p_tickets;
	    }
	  }
	  uint16_t lfsr = read_cycle_counter();
	  uint16_t bit;
	  bit = ((lfsr >> 0) ^ (lfsr >> 2)  
		 ^ (lfsr >> 3) ^ (lfsr >> 5)) & 1;
	  lfsr = (lfsr >> 1) | (bit << 15);
	  lfsr = lfsr % numTickets;
	  int currentTicket = 0;
	  pid = 1;
	  while(currentTicket < lfsr) {
	    if(proc_array[pid].p_state == P_RUNNABLE) {
	      currentTicket += proc_array[pid].p_tickets;
	    }
	    pid = (pid + 1) % NPROCS;
	    }
