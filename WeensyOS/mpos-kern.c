...
static pid_t do_thread (process_t *parent, void *start_function(void));
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
	  ...
	  for(j = 0; j < NPROCS; j++) {
		  if(proc_array[j].p_wait == current->p_pid) {
		    proc_array[j].p_registers.reg_eax = current->p_exit_status;
		    proc_array[j].p_wait = -1;
	...
	case INT_SYS_THREAD: {
	  current->p_registers.reg_eax = do_thread(current, (void*)current->p_registers.reg_ebx);
	  run(current);
	}

	case INT_SYS_KILL: {
	  pid_t p = current->p_registers.reg_eax;
	  process_t *t = &proc_array[p];
	  if (p <= 0 || p >= NPROCS || p == current->p_pid || proc_array[p].p_state == P_EMPTY)
	    current->p_registers.reg_eax = -1;
	  else if(t->p_state != P_ZOMBIE) {
	    t->p_state = P_ZOMBIE;
	    t->p_exit_status = -1;
	    int k;
	    for(k = 0; k < NPROCS; k++) {
	      if(proc_array[k].p_wait == p) {
		proc_array[k].p_registers.reg_eax = t->p_exit_status;
		proc_array[k].p_wait = -1;
...
static pid_t do_thread (process_t *parent, void *start_function(void)) {
  ...
  child->p_registers = parent->p_registers;
  child->p_registers.reg_eax = 0;
  child->p_registers.reg_eip = (int) start_function;
  child->p_state = P_RUNNABLE;
  return child->p_pid;
}
